...
const UsersTable = () => {
  ...
  const [currentDelete, setCurrentDelete] = useState({id:''})

  const handleDelete = async (id) => {
    try {
      const response = await deleteSocialWorkerRequest(id);
      console.log(response)
      if (response.status === 200) {
        setSocialWorkers(socialWorkers.filter((socialWorker) => socialWorker._id !== id))
        setOpenAlert(false)
  ...
  useEffect(() => {
    const listarUsuarios = async () => {
      try {
        setLoading(true)
        const response = await getSocialWorkersRequest()
        if (response.status === 200) {
    ...
    <Dialog open={openAlert} onClose={setOpenAlert} className="relative z-10">
      ...
      <div>
        <div className="flex flex-col items-center overflow-x-auto md:max-h-[35vh] mb-8">
          <span className="font-bold text-sm mb-2">ASISTENTES SOCIALES</span>
          <div className="inline-block w-[90vw] overflow-x-auto">
            <table className="min-w-full  text-sm font-light">
              ...
              >
                <tr>
                  <th className="px-6 py-2 sticky top-0">CI</th>
