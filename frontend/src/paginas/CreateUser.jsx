...
const CreateUser = () => {
  ...
  const handleRegisterAdmin = async () => {
    ...
    //   };
    //   const response = await fetch(
    //     `${import.meta.env.VITE_BACKEND_URL}/admin/admin`,
    //     options
    //   );
    //   const data = await response.json();
    //   if (response.ok) {
      ...
      <form className="max-w-sm mx-auto px-4 sm:px-0">
        ...
        <div className={`mb-5 ${isUser ? "hidden" : ""}`}>
          ...
          <input
            type="text"
            name="nombre_usuario"
            value={formAdmin.nombre_usuario}
        ...
        <div className="mb-5">
          ...
          <input
            type="email"
            name="email"
            value={isUser ? formUser.email : formAdmin.email}
            onChange={isUser ? handleChangeUser : handleChangeAdmin}
            className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
